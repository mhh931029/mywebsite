...
if (firefox) {
	showSubdiv2.style.width = show.offsetWidth * 0.573 + "px";
}else{
...
for (var i = 0; i < productLinks.length; i ++) {
	productLinks[i].onclick = function () {
		...
		if (show.id == "quankou") {
			...
			var tempTooth = generateTooth("","","","");
			productDetails.push(tempTooth);
		}else{
			productDetails.push(productName);
		}
					...
					if (i < 20) {
						if (clickPoint && Math.sqrt(Math.pow(clickPoint.x - toothIndicatorArray1[i].x, 2) + Math.pow(clickPoint.y - toothIndicatorArray1[i].y, 2)) <= toothIndicatorArray1[i].radius){
							if (type1Selected == "lianguan" && toothIndicatorArray1[i].selected == false){
								...
								var tooth = generateTooth(toothIndicatorArray1[i].id, "lianguan", "", "", "");
								...
								context.fillStyle = "blue";
								toothIndicatorArray1[i].selected = true;
								var tooth = generateTooth(toothIndicatorArray1[i].id, "danguan", "", "", "");
								productDetails.push(tooth);
								createLi(toothIndicatorArray1[i].id, "bise");
							}else{
								context.fillStyle = "lightgray";
					...
					if (i < 20) {
						if (clickPoint && Math.sqrt(Math.pow(clickPoint.x - toothIndicatorArray1[i].x, 2) + Math.pow(clickPoint.y - toothIndicatorArray1[i].y, 2)) <= toothIndicatorArray1[i].radius){
							if (toothIndicatorArray1[i].selected == false){
								...
								toothIndicatorArray1[i].selected = true;
								var tooth = generateTooth(toothIndicatorArray1[i].id, "", "", "");
								productDetails.push(tooth);
...
function createLi(idName, type){
	switch (type) {
		case "bise":
			...
			var nameOfBise = document.createElement("a");
			nameOfBise.innerText = "牙" + idName;
			input.style.width = 125 + "px";
			input.style.borderRadius = 5 + "px";
			input.style.backgroundColor = "lightgray";
			input.placeholder = "比色信息(选填)";
			biseLi.appendChild(nameOfBise);
...
//生成牙齿信息
function generateTooth(toothPosition, singleOrContinual, colorInfo, paiyaInfo){
	if (toothPosition == null) {
		toothPosition = "";
	}
	if (singleOrContinual == null) {
		singleOrContinual = "";
	}
	if (colorInfo == null) {
		colorInfo = "";
	...
	tooth.colorInfo = colorInfo;
	tooth.paiyaInfo = paiyaInfo;
	return tooth;
}
//关闭canvas按钮
...
function resetSubsOfShow(para, drawType) {
	switch(para){
		...
		case "partial":
			if (show.id == "quankou") {
				productDetails.splice(2, productDetails.length);
			}else{
